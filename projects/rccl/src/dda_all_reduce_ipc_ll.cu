 ...
 * All-reduce analogue of dda_all_gather_fabric_ll.cu, wired to the IPC path so
 * it can be validated on gfx942/gfx950 before the fabric variant lands.
...
static ncclResult_t ncclAllReduceDdaIpcLLTyped(
  ...
  INFO(
      NCCL_COLL,
      "DDA IPC AllReduce LL: nRanks=%d bytes=%zu nPk=%zu grid=%u block=%u epoch=%u",
      nRanks, bytes, nPk, grid.x, block.x, flag);

  // NRANKS_CT 8: unrolled (IPC clique is fixed at kDdaNranks); 0: runtime.
  switch (nRanks) {
  case 8:
...
bool ncclAllReduceDdaIpcLLEligible(
  ...
  // IPC path: single node, exactly kDdaNranks ranks, own handler present.
