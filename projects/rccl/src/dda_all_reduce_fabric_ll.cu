...
#include "algorithms/all_reduce/all_reduce_dda_ll128_twoshot.h"
...
using meta::comms::kDdaLL128ArTwoShotSlotWords;
...
using meta::comms::ddaLL128ArTwoShotMaxSlices;
...
static ncclResult_t ncclAllReduceDdaFabricLL128TwoShotTyped(const void* sendbuff, void* recvbuff, size_t count,
  ...
  const size_t slotWords = meta::comms::kDdaLL128ArTwoShotSlotWords;
  ...
  switch (nRanks) {
  case 4:
    meta::comms::ddaAllReduceTwoShotLL128<T, 4, kWordsPerThread><<<grid, block, 0, stream>>>(
    ...
    break;
  case 8:
    meta::comms::ddaAllReduceTwoShotLL128<T, 8, kWordsPerThread><<<grid, block, 0, stream>>>(
      peers, static_cast<T*>(recvbuff), static_cast<const T*>(sendbuff), shardBytes, comm->rank, nRanks, epochDev,
      epochLen, slices, slotWords, wireWordPerSlice, dataBytesPerSlice);
    break;
  default:
    meta::comms::ddaAllReduceTwoShotLL128<T, 0, kWordsPerThread><<<grid, block, 0, stream>>>(
      peers, static_cast<T*>(recvbuff), static_cast<const T*>(sendbuff), shardBytes, comm->rank, nRanks, epochDev,
      epochLen, slices, slotWords, wireWordPerSlice, dataBytesPerSlice);
    break;
  }
